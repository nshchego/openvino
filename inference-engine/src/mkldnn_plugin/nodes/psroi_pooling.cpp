...
public:
    ...
    explicit PSROIPoolingImpl(const std::shared_ptr<ngraph::Node>& op) {
        try {
            ...
            if (op->get_input_shape(0).size() != 4)
                THROW_IE_EXCEPTION << errorPrefix << " has first input with incorrect rank: " + op->get_input_shape(0).size();
            if (op->get_input_shape(1).size() != 2)
                THROW_IE_EXCEPTION << errorPrefix << " has second input with incorrect rank: " + op->get_input_shape(1).size();
            if (!noTrans && op->get_input_shape(2).size() != 4)
                THROW_IE_EXCEPTION << errorPrefix << " has third input with incorrect rank: " + op->get_input_shape(2).size();
    ...
    void unpackParams(const TensorDesc& srcDesc, const TensorDesc& dstDesc,
        ...
        if (inBlkDims.size() != expectedInBlockDimsSize)
            THROW_IE_EXCEPTION << errorPrefix << " has unexpected size of blocking dims in input (given " << inBlkDims.size() << ", expected "
                              << expectedInBlockDimsSize << ")";
        if (outBlkDims.size() != expectedOutBlockDimsSize)
            THROW_IE_EXCEPTION << errorPrefix << " has unexpected size of blocking dims in output (given " << outBlkDims.size() << ", expected "
