...
void MKLDNNGenericNode::getSupportedDescriptors() {
    if (!extFactory && impls.empty()) {
        THROW_IE_EXCEPTION << "Cannot get generic primitive for layer: " << getName() << " with type: " << getTypeStr();
    }
}
...
bool MKLDNNGenericNode::created(const MKLDNNExtensionManager::Ptr &extMgr) {
    if (ngraphOp && extMgr) {
        ...
        if (impls.empty()) {
            ...
            if (rc != InferenceEngine::OK) {
                THROW_IE_EXCEPTION << InferenceEngine::details::as_status << rc << resp.msg;
            }
