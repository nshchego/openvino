    ...
    explicit NonMaxSuppressionImpl(const std::shared_ptr<ngraph::Node>& op) {
        try {
            ...
            if (nms->get_input_size() < 2 || nms->get_input_size() > 6)
                THROW_IE_EXCEPTION << errorPrefix << "has incorrect number of input edges: " << nms->get_input_size();

            if (nms->get_output_size() < 1 || nms->get_output_size() > 3)
                THROW_IE_EXCEPTION << errorPrefix << "has incorrect number of output edges: " << nms->get_output_size();
            ...
            if (boxes_dims.size() != 3)
                THROW_IE_EXCEPTION << errorPrefix << "has unsupported 'boxes' input rank: " << boxes_dims.size();
            if (boxes_dims[2] != 4)
                THROW_IE_EXCEPTION << errorPrefix << "has unsupported 'boxes' input 3rd dimension size: " << boxes_dims[2];
            ...
            if (scores_dims.size() != 3)
                THROW_IE_EXCEPTION << errorPrefix << "has unsupported 'scores' input rank: " << scores_dims.size();

            if (num_batches != scores_dims[0])
                THROW_IE_EXCEPTION << errorPrefix << " num_batches is different in 'boxes' and 'scores' inputs";
            if (num_boxes != scores_dims[2])
                THROW_IE_EXCEPTION << errorPrefix << " num_boxes is different in 'boxes' and 'scores' inputs";
            ...
            if (valid_outputs_dims.size() != 1)
                THROW_IE_EXCEPTION << errorPrefix << "has unsupported 'valid_outputs' output rank: " << valid_outputs_dims.size();
            if (valid_outputs_dims[0] != 1)
                THROW_IE_EXCEPTION << errorPrefix << "has unsupported 'valid_outputs' output 1st dimension size: " << valid_outputs_dims[1];
...
private:
    ...
    void checkPrecision(const ngraph::element::Type &ngPrec, const std::vector<Precision> precList, const std::string name, const std::string type) {
        ...
        if (std::find(precList.begin(), precList.end(), prec) == precList.end())
            THROW_IE_EXCEPTION << errorPrefix << "has unsupported '" << name << "' " << type << " precision: " << prec;
    }
    ...
    void check1DInput(const std::shared_ptr<ngraph::Node>& op, const std::vector<Precision> precList, const std::string name, const size_t port) {
        ...
        if (dims.size() != 0 && dims.size() != 1)
            THROW_IE_EXCEPTION << errorPrefix << "has unsupported '" << name << "' input rank: " << dims.size();
        if (dims.size() == 1)
            if (dims[0] != 1)
                THROW_IE_EXCEPTION << errorPrefix << "has unsupported '" << name << "' input 1st dimension size: " << dims[0];
    }
    ...
    void checkOutput(const std::shared_ptr<ngraph::Node>& op, const std::vector<Precision> precList, const std::string name, const size_t port) {
        ...
        if (dims.size() != 2)
            THROW_IE_EXCEPTION << errorPrefix << "has unsupported '" << name << "' output rank: " << dims.size();
        if (dims[1] != 3)
            THROW_IE_EXCEPTION << errorPrefix << "has unsupported '" << name << "' output 2nd dimension size: " << dims[1];
    }
};
