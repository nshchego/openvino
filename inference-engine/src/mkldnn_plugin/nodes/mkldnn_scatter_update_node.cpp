...
void MKLDNNScatterUpdateNode::getSupportedDescriptors() {
    ...
    if (getParentEdgeAt(DATA_ID)->getDims().ndims() < 1 ||
        ...
        THROW_IE_EXCEPTION << errorPrefix << " do not support scalar input";
    }
    ...
    if (scatterUpdateType == ScatterUpdate) {
        ...
        THROW_IE_EXCEPTION << errorPrefix << " is not supported";
    }
}
...
void MKLDNNScatterUpdateNode::initSupportedPrimitiveDescriptors() {
    ...
    // common check
    if (srcRank != dstRank) {
        THROW_IE_EXCEPTION << errorPrefix << " should have same rank for input and output tensor";
    } else {
        for (size_t r = 0; r < srcRank; r++) {
            if (srcDataDim[r] != dstDataDim[r]) {
                THROW_IE_EXCEPTION << errorPrefix << " should have same shape for input and output tensor. The input shape is "
    ...
    switch (scatterUpdateMode) {
        case ScatterUpdateMode::ScatterUpdate: {
            if (updateRank != (srcRank + indicesRank - 1)) {
                THROW_IE_EXCEPTION << errorPrefix << " do not have matched tensor rank relationship for input, indices and update";
            }
            break;
        }
        case ScatterUpdateMode::ScatterNDUpdate: {
            ...
            if (k > srcRank) {
                THROW_IE_EXCEPTION << errorPrefix << "' do not have an correct indices' last dimension value, "
            ...
            for (size_t ru = 0; ru < updateRank; ru++) {
                if (updateDim[ru] != expectUpdateShape[ru]) {
                    THROW_IE_EXCEPTION << errorPrefix << " do not have matched tensor shape relationship for input, indices and update";
                }
            }
            break;
        }
        case ScatterUpdateMode::ScatterElementsUpdate: {
            if (srcRank != indicesRank || srcRank != updateRank) {
                THROW_IE_EXCEPTION << errorPrefix << " do not have the same tensor rank for input, indices and update";
            }
            for (size_t ri = 0; ri < indicesRank; ri++) {
                if (indicesDim[ri] != updateDim[ri]) {
                    THROW_IE_EXCEPTION << errorPrefix << " do not have the same tensor shape for indices and update";
                }
            }
            break;
        }
...
void MKLDNNScatterUpdateNode::createPrimitive() {
    ...
    if (!indicesMemPtr || !indicesMemPtr->GetPrimitivePtr())
        THROW_IE_EXCEPTION << errorPrefix << " did not allocate indices memory";
    if (!updateMemPtr || !updateMemPtr->GetPrimitivePtr())
        THROW_IE_EXCEPTION << errorPrefix << " did not allocate update memory";
