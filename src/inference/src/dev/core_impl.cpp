...
ov::SoPtr<ov::ICompiledModel> ov::CoreImpl::compile_model(const std::shared_ptr<const ov::Model>& model_,
    ...
    if (cacheManager && device_supports_model_caching(plugin) && !is_proxy_device(plugin)) {
        ...
        res = load_model_from_cache(cacheContent, plugin, parsed._config, ov::SoPtr<ov::IRemoteContext>{}, true/*coreConfig.get_enable_mmap()*/, [&]() {
...
ov::SoPtr<ov::ICompiledModel> ov::CoreImpl::compile_model(const std::shared_ptr<const ov::Model>& model_,
    ...
    if (cacheManager && device_supports_model_caching(plugin) && !is_proxy_device(plugin)) {
        ...
        cacheContent.blobId = ov::ModelCache::compute_hash(model, create_compile_config(plugin, parsed._config));
        auto lock = cacheGuard.get_hash_lock(cacheContent.blobId);
        res = load_model_from_cache(cacheContent, plugin, parsed._config, context, true/*coreConfig.get_enable_mmap()*/, [&]() {
...
ov::SoPtr<ov::ICompiledModel> ov::CoreImpl::compile_model(const std::string& model_path,
    ...
    if (cacheManager && device_supports_model_caching(plugin) && !is_proxy_device(plugin)) {
        ...
        compiled_model = load_model_from_cache(
            ...
            parsed._config,
            ov::SoPtr<ov::IRemoteContext>{},
            true,//coreConfig.get_enable_mmap(),
            [&]() {
    ...
    if (cacheManager && device_supports_model_caching(plugin) && !is_proxy_device(plugin)) {
        ...
        compiled_model = load_model_from_cache(
            ...
            [&]() {
                ...
                return compile_model_and_cache(plugin, model, parsed._config, ov::SoPtr<ov::IRemoteContext>{}, cacheContent);
            });
    } else {
        auto model = read_model(model_str, weights);
        compiled_model = plugin.compile_model(model, parsed._config);
