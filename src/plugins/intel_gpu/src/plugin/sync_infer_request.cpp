...
void SyncInferRequest::set_tensor(const ov::Output<const ov::Node>& port, const ov::SoPtr<ov::ITensor>& tensor) {
    ...
    if (port.get_element_type() != tensor->get_element_type()) {
        printf("\n");
    }
    OPENVINO_ASSERT(port.get_element_type() == tensor->get_element_type(),
                    "[GPU] Mismtach port and tensor types: ", port.get_element_type(), " vs ", tensor->get_element_type(),
                    " for node with name '", port.get_node()->get_friendly_name(), "'");
