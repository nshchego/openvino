...
protected:
    std::shared_ptr<ov::Model> createGraph(InferenceEngine::Precision netPrecision) override {
        ...
        // multiply
        auto input1 = std::make_shared<opset1::Parameter>(ntype, ov::Shape{1, 3, 224, 224});
        ...
        ov::Shape convFilterShape = { 3, 3, 3, 3 };  // out channel, /input channels, kernel h, kernel w
        if (netPrecision == Precision::FP32) {
        ...
        return std::make_shared<ov::Model>(ov::NodeVector{eltNode2}, ov::ParameterVector{input1});
    }
    void SetUp() override {
