...
protected:
    std::shared_ptr<ov::Model> createGraph(InferenceEngine::Precision netPrecision) override {
        ...
        // multiply
        auto input1 = std::make_shared<opset1::Parameter>(ov::element::f32, ov::Shape{inputShapes});
        auto const1 = opset1::Constant::create(ov::element::f32, Shape{1}, { 2.0f });
        auto mulNode = std::make_shared<opset1::Multiply>(input1, const1);

        // add
        auto const2 = opset1::Constant::create(ov::element::f32, Shape{1}, { 1.0f });
        ...
        auto weightsNode2 = std::make_shared<ov::op::v0::Constant>(ov::element::f32, convFilterShape2, weightValues2);
        ...
        return std::make_shared<ov::Model>(ov::NodeVector{convNode2}, ov::ParameterVector{input1});
    }
    void SetUp() override {
