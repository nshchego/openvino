...
INSTANTIATE_TEST_SUITE_P(
    ...
    Values(make_tuple(unit_test::ShapeVector{{3}, {1}}, std::vector<int64_t>{0}, StaticShape({3})),
           ...
           make_tuple(unit_test::ShapeVector{{10, 8, 9, 2}, {4}},
                      std::vector<int64_t>{2, 0, 3, 1},
                      StaticShape({9, 10, 2, 8})),
           make_tuple(unit_test::ShapeVector{{1, 2, 3, 4}, {4}},
...
TEST_P(TransposeCpuShapeInferenceThrowExceptionTest, shape_inference_in_const_map) {
    ...
    const auto const_tensor = transpose_order.empty()
                                  ? ov::Tensor(element::i64, ov::Shape{transpose_order.size()})
