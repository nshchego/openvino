...
GatherTree::GatherTree(const std::shared_ptr<ov::Node>& op, const GraphContext::CPtr& context)
    ...
    CPU_NODE_ASSERT(m_input_shapes.size() == 4, "has incorrect number of input edges.");
...
void GatherTree::save(BinaryOutputBuffer& out_buf) const {
    Node::save(out_buf);
...
void GatherTree::load(BinaryInputBuffer& in_buf) {
