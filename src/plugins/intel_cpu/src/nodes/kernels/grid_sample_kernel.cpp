...
jitUniGridSampleKernel<isa>::jitUniGridSampleKernel(const jGridSampleConfParams& jcp) :
        jitGridSampleKernelBase(jcp), x64::jit_generator() {
    ...
    if (jcp.interpolationMode == InterpolationMode::BILINEAR) {
        ...
        L(lChannelLoopBegin);
        {
            ...
            if (jcp.paddingMode == PaddingMode::ZEROS) {
                ...
                uni_vpgatherdd(vWCoord, ptr[regSrcIter + vAux2], vAux4); // v00 -> vWCoord TODO: 64b 16b 8b?
                ...
                uni_vpgatherdd(vAux3, ptr[regSrcIter + vAux2], vAux4); // TODO: 64b 16b 8b?
                uni_vmovups(vAux4, vAux5);
                if (jcp.inDataPrc == InferenceEngine::Precision::FP32) {
                ...
                uni_vpgatherdd(vHCoord, ptr[regSrcIter + vAux2], vAux4); // TODO: 64b 16b 8b?
                if (jcp.inDataPrc == InferenceEngine::Precision::FP32) {
...
void jitUniGridSampleKernel<isa>::calcCoordinates(const Vmm* vAuxPool, bool shiftFirst) {
    ...
    if (shiftFirst)
        add(regGridIter, vlen);

    if (isa == x64::avx512_core) {
        // Permute to XXXX.XXXX.YYYY.YYYY
        vpermd(vH, vPermIdxMask, ptr[regGrid + regGridIter]);
        ...
        uni_vmovups(vAux0, ptr[regGrid + regGridIter]);
        // Permute to XXXX.YYYY
        vpermd(vH, vPermIdxMask, vAux0);
        // Extract Y component
    ...
    if (isa == x64::avx512_core) {
        // Permute to XXXX.XXXX.YYYY.YYYY
        vpermd(vAux0, vPermIdxMask, ptr[regGrid + regGridIter]);
        ...
        uni_vmovups(vAux0, ptr[regGrid + regGridIter]);
        // Permute to XXXX.YYYY
        vpermd(vAux0, vPermIdxMask, vAux0);
