...
void JitReducePostKernel<isa>::reduce_post_main() {
    ...
    L(reduce_map_label);
    {
        if (jcp.reduce_mode == Algorithm::ReduceL2 || jcp.reduce_mode == Algorithm::ReduceMean ||
            jcp.reduce_mode == Algorithm::ReduceLogSum || jcp.reduce_mode == Algorithm::ReduceLogSumExp) {
