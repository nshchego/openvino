...
std::cout << "[CPU] Core output_prc: " << m_output_prc << std::endl;
...
std::cout << "[CPU] RandomUniform::RandomUniform m_min_val: " << m_min_val.f32 << std::endl;
    }
...
std::cout << "[CPU] RandomUniform::RandomUniform m_max_val: " << m_max_val.f32 << std::endl;
    }
...
void RandomUniform::prepareParams() {
    ...
    parallel_nt(m_threads_num, [&](const int ithr, const int nthr) {
        ...
        if (m_jit_kernel) {
            ...
            const auto blocks_num = (out_el_num + block_size - 1) / block_size;
            ...
            p.start = ithr * groups_per_thr * PHILOX_GROUP_SIZE;
            p.end = (ithr + 1) * groups_per_thr * PHILOX_GROUP_SIZE;
            if (p.end > out_el_num) {
                p.end = out_el_num;
            }
            if (p.start > p.end) {
            ...
            p.work_amount = p.end - p.start;
            p.n_shift = p.start / PHILOX_GROUP_SIZE;
...
void RandomUniform::execute(dnnl::stream strm) {
    ...
    if (m_algo == PHILOX) {
        m_state = computePhilox(dstMemPtr->getData(), out_el_num, m_state);
    } else if (m_algo == STL) {
...
std::pair<uint64_t, uint64_t> RandomUniform::computePhilox(void* out, size_t out_el_num, const std::pair<uint64_t, uint64_t>& prev_state) {
    ...
    if (m_jit_kernel) {
        ...
        // const auto groups_num = (out_el_num + PHILOX_GROUP_SIZE - 1) / PHILOX_GROUP_SIZE;
        // const auto step = m_output_prc.size() > 4 ? 2 : 4;
...
// printf("[CPU][%d] exec out_el_num: %ld; step: %ld; start: %ld; end: %ld\n", ithr, out_el_num, step, start, end);
            ...
            uint32_t res[4];
            for (size_t k = p.start; k < p.end; k += p.step) {
...
// printf("[CPU][%d] key: %lu; counter: %ld; n: %ld; res={%s}\n", ithr, key, counter, n, res_str.c_str());
                ...
                switch (m_output_prc) {
                    case element::f32: {
                        convertToOutputType<float>(res, p.step, m_output_prc, m_min_val.f32, m_max_val.f32, out_u8, k, out_el_num, uint32ToFloat);
                    } break;
                    case element::f16: {
