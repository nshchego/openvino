...
Pad::Pad(const std::shared_ptr<ov::Node>& op, const GraphContext::CPtr& context)
    ...
    CPU_NODE_ASSERT(any_of(m_input_shapes.size(), 3U, 4U), "has incorrect number of input edges");
    CPU_NODE_ASSERT(m_output_shapes.size() == 1, "Incorrect number of output edges");
...
Pad::Pad(BinaryInputBuffer& in_buf, const GraphContext::CPtr& context)
    : Node(in_buf, context) {
    load(in_buf);
}
