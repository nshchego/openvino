...
StridedSlice::StridedSlice(const std::shared_ptr<ov::Node>& op, const GraphContext::CPtr& context)
    ...
    const bool wrong_size_for_strided = m_input_shapes.size() < 3 || m_input_shapes.size() > 4;
...
StridedSlice::StridedSlice(BinaryInputBuffer& in_buf, const GraphContext::CPtr& context)
    : Node(in_buf, context) {
    load(in_buf);
}
