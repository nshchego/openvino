...
ReverseSequence::ReverseSequence(const std::shared_ptr<ov::Node>& op, const GraphContext::CPtr& context)
    ...
    CPU_NODE_ASSERT(m_input_shapes.size() == 2 && m_output_shapes.size() == 1, "has incorrect number of input/output edges!");
...
ReverseSequence::ReverseSequence(BinaryInputBuffer& in_buf, const GraphContext::CPtr& context)
    : Node(in_buf, context) {
    load(in_buf);
}
