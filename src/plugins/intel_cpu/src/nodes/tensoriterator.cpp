...
TensorIterator::TensorIterator(BinaryInputBuffer& in_buf, const GraphContext::CPtr& context)
    : Node(in_buf, context) {
    load(in_buf);
}
...
void TensorIterator::createPrimitive() {
    ...
    for (const auto& param : subgraphOp->get_function()->get_parameters()) {
        auto inNode = sub_graph.getInputNodeByIndex(subgraphOp->get_function()->get_parameter_index(param));
        input_mems.push_back(getToMemories(inNode.get(), 0));
    }

    for (const auto& out : subgraphOp->get_function()->get_results()) {
        auto outNode = sub_graph.getOutputNodeByIndex(subgraphOp->get_function()->get_result_index(out));
...
int TensorIterator::getNumIteration(const std::vector<PortMap>& inputPortMap,
    ...
    for (const auto& rule : inputPortMap) {
        CPU_NODE_ASSERT(rule.from >= 0 && rule.from < static_cast<int64_t>(m_input_shapes.size()),
    ...
    for (const auto& rule : outputPortMap) {
        ...
        CPU_NODE_ASSERT(rule.from >= 0 && rule.from < static_cast<int64_t>(m_output_shapes.size()),
                        ": Invalid \"from\" value: \"from\" = ",
                        rule.from,
                        " inputs number = ",
