...
//        const auto execPrcLen = sizeof(exec_precisions_priority) / sizeof(*exec_precisions_priority);

//std::cout << "exec_precisions_priority size: " << execPrcLen << std::endl;
//        std::vector<Precision> inPrecisions;
//        for (int i = execPrcLen - 1; i >= 0; i--) {
//            for (int j = 0; j < jep_.inputs_number; j++) {
//                if (jep_.src_prc[j] == exec_precisions_priority[i]) {
//                    inPrecisions.push_back(exec_precisions_priority[i]);
//                }
//            }
//        }
