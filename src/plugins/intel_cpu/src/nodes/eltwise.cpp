...
void Eltwise::initSupportedPrimitiveDescriptors() {
    ...
    const auto preferredLayout =
        m_context->getConfig().modelType == Config::ModelType::CNN ? LayoutType::nspc : LayoutType::ncsp;
    ...
    auto executionContext = std::make_shared<ExecutorContext>(m_context, getImplPriority());
    ...
    sortSupportedPrimitiveDescriptors(supportedPrimitiveDescriptors, m_context->getConfig().modelType);
}
...
bool Eltwise::canFuse(const NodePtr& node) const {
    ...
    if (!EltwiseJitExecutor::supports(m_attrs, m_input_shapes.front().getRank())) {
        return false;
    }
...
//     return Node::needPrepareParams() || !m_executor;
// }
...
bool Eltwise::canFuseParent(const NodePtr& parentNode) const {
    ...
    return EltwiseJitExecutor::supports(m_attrs, m_input_shapes.front().getRank(), input_precisions);
#else
