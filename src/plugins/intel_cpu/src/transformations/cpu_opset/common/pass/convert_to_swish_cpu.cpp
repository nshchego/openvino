...
ov::intel_cpu::ConvertToSwishCPU::ConvertToSwishCPU() {
    ...
    auto swish = ngraph::pattern::wrap_type<op::v4::Swish>();

    ngraph::matcher_pass_callback callback = [](ngraph::pattern::Matcher& m) {
        auto swish = ov::as_type_ptr<op::v4::Swish> (m.get_match_root());
        if (!swish) {
            return false;
        }
        float beta_value = 1.0;
        if (swish->input_values().size() == 2) {
            auto beta = ov::as_type<ov::op::v0::Constant>(swish->get_input_node_ptr(1));
