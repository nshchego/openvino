...
bool MHAFusionBase::valid_transpose_order(const std::shared_ptr<ov::Node>& node, const std::vector<int64_t>& expected_order) {
    if (auto transpose_pattern = as_type_ptr<ov::op::v0::Constant>(node)) {
...
MHAFloatFusion::MHAFloatFusion() {
    ...
    auto in0 = pass::pattern::any_input(ov::pass::pattern::has_static_shape());
...
//std::cout << "MHAFloatFusion::callback" << std::endl;
        ...
        if (auto mul_node = as_type_ptr<ov::op::v1::Multiply>(pattern_to_output.at(mul).get_node_shared_ptr())) {
            mul_scales = as_type_ptr<ov::op::v0::Constant>(mul_node->get_input_node_shared_ptr(1))->cast_vector<float>();
        ...
        auto matmul0_node = as_type_ptr<ov::op::v0::MatMul>(pattern_to_output.at(matmul0).get_node_shared_ptr());
        if (!matmul0_node)
            return false;
        if (matmul0_node->get_transpose_a() || matmul0_node->get_transpose_b())
            return false;

        auto reshape0_node = as_type_ptr<ov::op::v1::Reshape>(pattern_to_output.at(reshape0).get_node_shared_ptr());
        if (!reshape0_node)
            return false;

        if (auto reshape_pattern = as_type_ptr<ov::op::v0::Constant>(pattern_to_output.at(in6).get_node_shared_ptr())) {
        ...
        if (auto reshape1_node = as_type_ptr<ov::op::v1::Reshape>(pattern_to_output.at(reshape1).get_node_shared_ptr())) {
        ...
        auto softmax_node = as_type_ptr<ov::op::v1::Softmax>(pattern_to_output.at(softmax).get_node_shared_ptr());
        if (!softmax_node)
            return false;
        if (softmax_node->get_axis() != 1)
            return false;

        auto matmul1_node = as_type_ptr<ov::op::v0::MatMul>(pattern_to_output.at(matmul1).get_node_shared_ptr());
        if (!matmul1_node)
            return false;
...
MHAQuantFusion::MHAQuantFusion() {
    ...
    auto transpose1 = std::make_shared<ov::op::v1::Transpose>(in1, in5);
    auto matmul0 = std::make_shared<ov::op::v0::MatMul>(transpose0, transpose1);
    auto fakeQuantize0 = ov::pass::pattern::wrap_type<ov::op::v0::FakeQuantize>({matmul0,
    ...
    auto softmax = std::make_shared<ov::op::v1::Softmax>(reshape0);
    auto reshape1 = std::make_shared<ov::op::v1::Reshape>(softmax, in7, true);
    auto fakeQuantize1 = ov::pass::pattern::wrap_type<ov::op::v0::FakeQuantize>({reshape1,
                                                                                   ov::pass::pattern::wrap_type<ov::op::v0::Constant>(),
                                                                                   ov::pass::pattern::wrap_type<ov::op::v0::Constant>(),
                                                                                   ov::pass::pattern::wrap_type<ov::op::v0::Constant>(),
                                                                                   ov::pass::pattern::wrap_type<ov::op::v0::Constant>()});
    ...
    auto fakeQuantize2 = ov::pass::pattern::wrap_type<ov::op::v0::FakeQuantize>({matmul1,
                                                                                   ov::pass::pattern::wrap_type<ov::op::v0::Constant>(),
                                                                                   ov::pass::pattern::wrap_type<ov::op::v0::Constant>(),
                                                                                   ov::pass::pattern::wrap_type<ov::op::v0::Constant>(),
                                                                                   ov::pass::pattern::wrap_type<ov::op::v0::Constant>()});
    ...
    matcher_pass_callback callback = [=](ov::pass::pattern::Matcher& m) {
        ...
        auto fq0_node = as_type_ptr<ov::op::v0::FakeQuantize>(pattern_to_output.at(fakeQuantize0).get_node_shared_ptr());
        if (fq0_node) {
        ...
        auto fq1_node = as_type_ptr<ov::op::v0::FakeQuantize>(pattern_to_output.at(fakeQuantize1).get_node_shared_ptr());
        if (fq1_node) {
        ...
        if (auto fq_node = as_type_ptr<ov::op::v0::FakeQuantize>(pattern_to_output.at(fakeQuantize2).get_node_shared_ptr())) {
...
MHAQuantFusion2::MHAQuantFusion2() {
    ...
    auto transpose0 = std::make_shared<ov::op::v1::Transpose>(in0, in4);
    auto transpose1 = std::make_shared<ov::op::v1::Transpose>(in1, in5);
    auto fakeQuantize0 = ov::pass::pattern::wrap_type<ov::op::v0::FakeQuantize>({transpose1,
                                                                                ov::pass::pattern::wrap_type<ov::op::v0::Constant>(),
                                                                                ov::pass::pattern::wrap_type<ov::op::v0::Constant>(),
                                                                                ov::pass::pattern::wrap_type<ov::op::v0::Constant>(),
                                                                                ov::pass::pattern::wrap_type<ov::op::v0::Constant>()});
    ...
    auto transpose2 = std::make_shared<ov::op::v1::Transpose>(in8, in9);
    auto matmul1 = std::make_shared<ov::op::v0::MatMul>(softmax, transpose2);
    auto fakeQuantize1 = ov::pass::pattern::wrap_type<ov::op::v0::FakeQuantize>({matmul1,
                                                                                   ov::pass::pattern::wrap_type<ov::op::v0::Constant>(),
                                                                                   ov::pass::pattern::wrap_type<ov::op::v0::Constant>(),
                                                                                   ov::pass::pattern::wrap_type<ov::op::v0::Constant>(),
                                                                                   ov::pass::pattern::wrap_type<ov::op::v0::Constant>()});
    auto transpose3 = std::make_shared<ov::op::v1::Transpose>(fakeQuantize1, in10);
    ...
    matcher_pass_callback callback = [=](ov::pass::pattern::Matcher& m) {
        ...
        std::vector<float> fq1_scale;
        if (auto fq_node = as_type_ptr<ov::op::v0::FakeQuantize>(pattern_to_output.at(fakeQuantize1).get_node_shared_ptr())) {
