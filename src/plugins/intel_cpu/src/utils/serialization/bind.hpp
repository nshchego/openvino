...
template <typename BufferType>
using dif = loader_storage<BufferType, std::function<void(BufferType&, std::unique_ptr<void, void_deleter<void>>&, dnnl::engine&)>>;
...
private:
    buffer_binder() {
        dif<BufferType>::instance().set_load_function(
                {T::get_type_info_s(), [](BufferType& buffer, std::unique_ptr<void, void_deleter<void>>& result_ptr, dnnl::engine& engine) {
