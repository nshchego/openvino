...
template <typename BufferType, typename Key, typename Value, typename Hash>
class Serializer<BufferType, std::unordered_map<Key, Value, Hash>, typename std::enable_if<std::is_base_of<OutputBuffer<BufferType>, BufferType>::value>::type> {
public:
    static void save(BufferType& buffer, const std::unordered_map<Key, Value, Hash>& map) {
// printf("-WRITE unordered_map-\n");
        const auto map_size = map.size();
        buffer.write(std::addressof(map_size), sizeof(map_size));
        for (const auto& pair : map) {
...
template <typename BufferType, typename Key, typename Value, typename Hash>
class Serializer<BufferType, std::unordered_map<Key, Value, Hash>, typename std::enable_if<std::is_base_of<InputBuffer<BufferType>, BufferType>::value>::type> {
public:
    static void load(BufferType& buffer, std::unordered_map<Key, Value, Hash>& map) {
// printf("-READ unordered_map-\n");
        typename std::unordered_map<Key, Value, Hash>::size_type map_size = 0UL;
        buffer.read(std::addressof(map_size), sizeof(map_size));
        map.clear();
        Key key;
        for (size_t i = 0; i < map_size; i++) {
