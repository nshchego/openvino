...
protected:
    ...
    Node(BinaryInputBuffer& in_buf, const GraphContext::CPtr& ctx, const ShapeInferFactory& shapeInferFactory);

    Node(BinaryInputBuffer& in_buf, const GraphContext::CPtr& ctx);
...
struct NodeImpl : public NodeType {
    ...
    NodeImpl(BinaryInputBuffer& in_buf, const GraphContext::CPtr& context) : NodeType(in_buf, context) {
        NodeType::perfCounters().template buildClassCounters<NodeType>(NameFromType(NodeType::getType()));
    }
};
