...
std::shared_ptr<ov::Model> dump_graph_as_ie_ngraph_net(const Graph& graph) {
    ...
    auto create_ngraph_node = [&](const NodePtr& node) {
        auto found_input = std::find(graph.m_input_nodes.begin(), graph.m_input_nodes.end(), node);
        const auto is_input = found_input != graph.m_input_nodes.end();
        auto found_output = std::find(graph.m_output_nodes.begin(), graph.m_output_nodes.end(), node);
        const auto is_output = found_output != graph.m_output_nodes.end();
        ...
        if (is_input) {
            ...
            return_node = param;
            const auto input_index = std::distance(graph.m_input_nodes.begin(), found_input);
            paramsMap[input_index] = param;
        } else if (is_output) {
            auto result = std::make_shared<ov::op::v0::Result>(get_inputs(node).back());
            const auto output_index = std::distance(graph.m_output_nodes.begin(), found_output);
