...
std::vector<std::string> XmlSerializer::map_type_from_body(const pugi::xml_node& xml_node,
...
void XmlSerializer::input_descriptions_on_adapter(
    ...
    for (const auto& input_description : input_descriptions) {
        ...
        if (auto slice_input = ov::as_type_ptr<ov::op::util::SubGraphOp::SliceInputDescription>(input_description)) {
...
void XmlSerializer::output_descriptions_on_adapter(
...
void XmlSerializer::special_body_ports_on_adapter(const ov::op::v5::Loop::SpecialBodyPorts& special_body_ports,
                                                  const std::vector<std::string>& parameter_mapping,
                                                  const std::vector<std::string>& result_mapping,
                                                  pugi::xml_node& port_map) {
...
bool XmlSerializer::append_rt_attribute(pugi::xml_node& node, const ov::RuntimeAttribute& attribute) {
    ...
    rt_info::RTInfoSerializer serializer(node);
    return attribute.visit_attributes(serializer);
}

void XmlSerializer::append_rt_info(pugi::xml_node& node, ov::RTMap& attributes) {
    ...
    for (auto& item : attributes) {
        if (item.second.is<ov::RuntimeAttribute>()) {
            ...
            if (append_rt_attribute(attribute_node, rt_attribute)) {
                has_attrs = true;
            } else {
...
XmlSerializer::XmlSerializer(pugi::xml_node& data,
    ...
    : m_xml_node(data),
      ...
      m_constant_node_write_handler(std::ref(constant_write_handler)),
      m_version(version),
...
void XmlSerializer::on_adapter(const std::string& name, ov::ValueAccessor<void>& adapter) {
    ...
    if (is_body_target) {
        ...
        const auto& attrs = a->get();
        m_xml_node.append_attribute(name.c_str()).set_value(util::join(attrs).c_str());
...
void XmlSerializer::on_adapter(const std::string& name, ov::ValueAccessor<bool>& adapter) {
    m_xml_node.append_attribute(name.c_str()).set_value(adapter.get());
}

void XmlSerializer::on_adapter(const std::string& name, ov::ValueAccessor<std::string>& adapter) {
    std::string value;
...
void XmlSerializer::on_adapter(const std::string& name, ov::ValueAccessor<int64_t>& adapter) {
    m_xml_node.append_attribute(name.c_str()).set_value(static_cast<long long>(adapter.get()));
}

void XmlSerializer::on_adapter(const std::string& name, ov::ValueAccessor<double>& adapter) {
    m_xml_node.append_attribute(name.c_str()).set_value(adapter.get());
}

void XmlSerializer::on_adapter(const std::string& name, ov::ValueAccessor<std::vector<int>>& adapter) {
    m_xml_node.append_attribute(name.c_str()).set_value(create_attribute_list(adapter).c_str());
}

void XmlSerializer::on_adapter(const std::string& name, ov::ValueAccessor<std::vector<int64_t>>& adapter) {
    m_xml_node.append_attribute(name.c_str()).set_value(create_attribute_list(adapter).c_str());
}
void XmlSerializer::on_adapter(const std::string& name, ov::ValueAccessor<std::vector<uint64_t>>& adapter) {
    m_xml_node.append_attribute(name.c_str()).set_value(create_attribute_list(adapter).c_str());
}

void XmlSerializer::on_adapter(const std::string& name, ov::ValueAccessor<std::vector<float>>& adapter) {
    m_xml_node.append_attribute(name.c_str()).set_value(create_attribute_list(adapter).c_str());
}

void XmlSerializer::on_adapter(const std::string& name, ov::ValueAccessor<std::vector<std::string>>& adapter) {
    m_xml_node.append_attribute(name.c_str()).set_value(create_attribute_list(adapter).c_str());
}

void XmlSerializer::on_adapter(const std::string& name, ov::ValueAccessor<std::shared_ptr<ov::Model>>& adapter) {
    if (name.find("body") != std::string::npos) {
        ...
        serialize(xml_body, *adapter.get());
        xml_body.remove_attribute("name");
        xml_body.remove_attribute("version");
    } else if (name == "net") {
        serialize(m_xml_node, *adapter.get());
    } else {
        OPENVINO_THROW("Unsupported Model name.");
    }
}

std::unique_ptr<XmlSerializer> XmlSerializer::make_visitor(pugi::xml_node& data,
                                                           const std::string& node_type_name,
                                                           ov::util::ConstantWriter& constant_write_handler,
                                                           int64_t version,
                                                           bool deterministic,
...
void XmlSerializer::serialize(pugi::xml_node& net_xml, const ov::Model& model) {
    ...
    // get_ordered_ops() returns operations after a topological sort. The topological sort reverses order of
    // Parameters and Results. So we need to put them into sorted_ops separately to ensure correct order of inputs
    // and outputs.
    {
    ...
    for (const auto& n : sorted_ops) {
        ...
        if (node->get_input_size() > 0) {
            ...
            for (auto& i : node->inputs()) {
                ...
                port.append_attribute("precision").set_value(get_ir_precision_name(port_element_type).c_str());
                ...
                if (m_version >= 11) {
                    append_rt_info(port, i.get_rt_info());
                }
            }
        ...
        if (node->get_output_size() > 0) {
            ...
            if (ov::op::util::is_output(node)) {
                if (m_version > 10 && !m_deterministic) {
                ...
                for (auto& o : node->outputs()) {
                    ...
                    if (m_version >= 11) {
                        append_rt_info(port, o.get_rt_info());
                    }
                }
                if (node_type_name == "TensorIterator" || node_type_name == "Loop") {
        ...
        // <layers/data> general attributes
        {
            ...
            auto visitor = make_visitor(data,
                                        node_type_name,
